<?php  
    ...
    while($row = mysqli_fetch_assoc($result)){
        if(isset($row['u_id'])){
            ...
            if($flag == 0){
                ...
                $insert = TRUE;
                $row4="UPDATE `license` SET `points` = `points`-5;";
                $con->query($row4);
                }
            }
        }
	...
	if($con->query($sql) == TRUE){
        $insert = TRUE;
        if($cd=='Not Wearing Helmet'){
            $row1="UPDATE `license` SET `points` = `points`-1;";
            $con->query($row1);
        }
        else if($cd=='Overspeeding'){
            $row1="UPDATE `license` SET `points` = `points`-2;";
            $con->query($row1);
        }
        else if($cd=='Accident'){
            $row1="UPDATE `license` SET `points` = `points`-3;";
            $con->query($row1);
        }
    }
	else {
